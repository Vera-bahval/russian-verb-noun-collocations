...
*В ответе напиши ТОЛЬКО глагол, больше ничего. Твой ответ должен быть ТОЛЬКО на русском языке.*"""
    ...
    for col, expected in zip(test_cols, expected_paraphrases):
        result = get_answer_mistral(model, tokenizer, prompt_paraphrase, col)
...
def main():
    argument_parser = ArgumentParser(description="Перефразирование коллокаций с помощью Mistral-7B-Instruct")
